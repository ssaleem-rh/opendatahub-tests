...
upgrade_phase: post
---
# TC-API-006: Submit evaluation job with public repo without secret_ref

**Objective**: Verify that the POST job submission API accepts a
`test_data_ref.git` payload for a public repository without
requiring a `secret_ref` field.

**Preconditions**:

- EvalHub API v0.4.0+ deployed with git storage source support
- Public test repository `evalhub-test-data-public` is accessible
  without authentication

**Test Steps**:

1. Send a POST request to `/api/v1/evaluations/jobs` with a
   `test_data_ref.git` payload that omits `secret_ref`
...
- Response body includes the submitted `test_data_ref.git`
  configuration without a `secret_ref` field
